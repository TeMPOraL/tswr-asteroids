...
(defmethod p2de:do-system ((system collision-detector) entity dt)
  (declare (ignore dt))
  (p2dprof:with-profiling ('do-system-collision-detector :description "collision checks time per frame")
    (p2dprof:with-counter (collision-counter 'collision-checks-per-frame :description "total collision checks per frame")
      (dolist (collision-layer (layers-that-can-collide-with (slot-value (p2de:find-component entity 'collision-sphere) 'layer)))
        (let ((others (entities-for-collision-layer collision-layer)))
          (p2dprof:increment-counter collision-counter (length others))
